...
$(function(){
  setModuleParams(); // probably need to run this in the pages it specifically applies to
});
...
function setModuleParams () {
  ...
  $('.vertical-neck-module-wrapper').each(function(){
    ...
    if($(this).find('.meta').length) {
      ...
      chordNecks[containerID].initFirstChord();

      chordNecks[containerID].resetNoteMetas();

      chordNecks[containerID].wrapper.attr('contenteditable','false');
